...
class ComFun{
	 ...
	 * 随机明文 md5 16位
	 */
	public static function getRandom($len=10){
	 ...
	 * 时间戳随机
	 */
	public static function getTimeRandom($len=10){
		$srcstr = md5(time());
	...
	public function getShortUrl ( $url ) {
		$_rb = DBCurl::dbGet( 'http://dbo.so/?url=' . $url . '&format=json ' );
		if ( $_rb['data'] ) {
			$_iurl = $_rb['data']['s_url'];
		} else {
			$_iurl = $url;
		}
		return $_iurl;
